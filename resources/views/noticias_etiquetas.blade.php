...
<div class="container-xxl container-fluid">
    ...
    <div class="row">
        ...
        <div class="col-12">
            <h2 class="Noticias-title" style="margin-top:3%;">Noticias {{$noticia_tag_title[0]['noticia_tag']}} </h2> 
            <hr>
        </div>
                        ...
                        <div class="col-md-6 col-12" style="text-align:center; display:flex; flex-wrap:wrap;">
                            <div class="noticia-col shadow-box p-10 mt-4">
                                <div class="bloque-noticias mb-2">
                                    <a href="{{asset('noticias/'.$nt->url)}}">
                                        <img src="{{asset($nt->imgnoticia)}}" href="{{asset('noticias/'.$nt->url)}}" alt="{{$nt->noticia}}" class="img-fluid">
                                    </a>
                                </div>
                                <a href="{{asset('noticias/'.$nt->url)}}" class="bloque-titulonoticia">{{substr($nt->noticia,0,70)}}</a>
                            </div>
                        </div>  
                    @endforeach
                </div>

                {{ $noticiasxtag->appends(request()->query())->links('front-partials.pagination-front') }}
                    ...
                    <div class="row mb-3 mt-25">
                        ...
                        <div class="col-lg-12 col-md-12 col-12 mt-3">
                            ...
                            @isset($noticias_etiquetas)
                                ...
                                <li class="{{ $ne['url'] == $url ? 'tag-style tag-style-style' : ''}}"><a href="{{url('noticia_etiquetas/'.$ne->url)}}" title="{{$ne->noticia_tag}}">{{$ne->noticia_tag}}</a></li>
                                @endforeach
                            @endisset
